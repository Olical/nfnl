...
local function fennel_filetype_callback(ev)
  ...
  if config0 then
    ...
    return vim.api.nvim_create_autocmd({"BufWritePost"}, {group = vim.api.nvim_create_augroup(("nfnl-dir-" .. root_dir), {}), pattern = core.map(_3_, cfg({"source-file-patterns"})), callback = fennel_buf_write_post_callback_fn(root_dir, cfg)})
  else
    return nil
  end
end
