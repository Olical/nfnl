...
local function default(opts)
  ...
  return {["compiler-options"] = {["error-pinpoint"] = false}, ["root-dir"] = root_dir, ["fennel-path"] = str.join(";", core.mapcat(_13_, dirs)), ["fennel-macro-path"] = str.join(";", core.mapcat(_14_, dirs)), ["source-file-patterns"] = {"*.fnl", fs["join-path"]({"**", "*.fnl"})}, ["fnl-path->lua-path"] = fs["fnl-path->lua-path"], verbose = false}
end
