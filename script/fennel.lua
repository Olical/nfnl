...
package.preload["fennel.binary"] = package.preload["fennel.binary"] or function(...)
  ...
  local _852_ = require("fennel.utils")
  local copy = _852_["copy"]
  local warn = _852_["warn"]
  ...
  local function execute(cmd)
    local _853_0 = os.execute(cmd)
    if (_853_0 == 0) then
      return true
    elseif (_853_0 == true) then
      return true
    end
  end
  ...
  local function module_name(open, rename, used_renames)
    ...
    do
      local _858_0 = rename[open]
      if (nil ~= _858_0) then
        local renamed = _858_0
  ...
  local function fennel__3ec(filename, native, options)
    ...
    local _862_
    do
      _862_ = "(do (local bundle_2_ ...) (fn loader_3_ [name_4_] (match (or (. bundle_2_ name_4_) (. bundle_2_ (.. name_4_ \".init\"))) (mod_5_ ? (= \"function\" (type mod_5_))) mod_5_ (mod_5_ ? (= \"string\" (type mod_5_))) (assert (if (= _VERSION \"Lua 5.1\") (loadstring mod_5_ name_4_) (load mod_5_ name_4_))) nil (values nil (: \"\n\\tmodule '%%s' not found in fennel bundle\" \"format\" name_4_)))) (table.insert (or package.loaders package.searchers) 2 loader_3_) ((assert (loader_3_ \"%s\")) ((or unpack table.unpack) arg)))"
    end
    fennel_loader = _862_:format(dotpath_noextension)
    local lua_loader = fennel["compile-string"](fennel_loader)
    local _863_ = options
    local rename_modules = _863_["rename-modules"]
  ...
  local function compile_binary(lua_c_path, executable_name, static_lua, lua_include_dir, native)
    ...
    local _865_
    do
      local _864_0 = shellout((cc .. " -dumpmachine"))
      if (nil ~= _864_0) then
        _865_ = _864_0:match("mingw")
      else
        _865_ = _864_0
      end
    end
    ...
    compile_command = {cc, "-Os", lua_c_path, table.concat(native, " "), static_lua, rdynamic, "-lm", _868_, "-o", (executable_name .. bin_extension), "-I", lua_include_dir, os.getenv("CC_OPTS")}
  ...
  local function native_path_3f(path)
    ...
    if (version_extension and (version_extension ~= "") and not version_extension:match("%.%d+")) then
      ...
      local _873_0 = extension
      if (_873_0 == "a") then
        return path
      elseif (_873_0 == "o") then
        return path
      elseif (_873_0 == "so") then
        return path
      elseif (_873_0 == "dylib") then
        return path
      else
        local _ = _873_0
        return false
  ...
  local function compile(filename, executable_name, static_lua, lua_include_dir, options, args)
    local _880_ = extract_native_args(args)
    local libraries = _880_["libraries"]
    local modules = _880_["modules"]
    local rename_modules = _880_["rename-modules"]
...
local function handle_lua(i)
  ...
  local _827_0, _828_0 = os.execute(table.concat(cmd, " "))
  if (((_827_0 == true) and (_828_0 == "exit")) or (_827_0 == 0)) then
...
do
  ...
  while (arg[i] and not options["ignore-options"]) do
    local _833_0 = arg[i]
    if (_833_0 == "--no-searcher") then
      options["no-searcher"] = true
      table.remove(arg, i)
    elseif (_833_0 == "--indent") then
      options.indent = table.remove(arg, (i + 1))
      if (options.indent == "false") then
        options.indent = false
      end
      table.remove(arg, i)
    elseif (_833_0 == "--add-package-path") then
      local entry = table.remove(arg, (i + 1))
      package.path = (entry .. ";" .. package.path)
      table.remove(arg, i)
    elseif (_833_0 == "--add-package-cpath") then
      local entry = table.remove(arg, (i + 1))
      package.cpath = (entry .. ";" .. package.cpath)
      table.remove(arg, i)
    elseif (_833_0 == "--add-fennel-path") then
      local entry = table.remove(arg, (i + 1))
      fennel.path = (entry .. ";" .. fennel.path)
      table.remove(arg, i)
    elseif (_833_0 == "--add-macro-path") then
      local entry = table.remove(arg, (i + 1))
      fennel["macro-path"] = (entry .. ";" .. fennel["macro-path"])
      table.remove(arg, i)
    elseif (_833_0 == "--load") then
      handle_load(i)
    elseif (_833_0 == "-l") then
      handle_load(i)
    elseif (_833_0 == "--no-fennelrc") then
      options.fennelrc = false
      table.remove(arg, i)
    elseif (_833_0 == "--correlate") then
      options.correlate = true
      table.remove(arg, i)
    elseif (_833_0 == "--check-unused-locals") then
      options.checkUnusedLocals = true
      table.remove(arg, i)
    elseif (_833_0 == "--globals") then
      allow_globals(table.remove(arg, (i + 1)), _G)
      table.remove(arg, i)
    elseif (_833_0 == "--globals-only") then
      allow_globals(table.remove(arg, (i + 1)), {})
      table.remove(arg, i)
    elseif (_833_0 == "--require-as-include") then
      options.requireAsInclude = true
      table.remove(arg, i)
    elseif (_833_0 == "--assert-as-repl") then
      options.assertAsRepl = true
      table.remove(arg, i)
    elseif (_833_0 == "--skip-include") then
      ...
      table.remove(arg, i)
    elseif (_833_0 == "--use-bit-lib") then
...
local function eval(form)
  local _843_
  if (form == "-") then
    _843_ = (io.stdin):read("*a")
  else
    _843_ = form
  end
  return print(dosafely(fennel.eval, _843_, options))
end
local function compile(files)
  for _, filename in ipairs(files) do
    ...
    do
      local _846_0, _847_0 = nil, nil
      local function _848_()
        return fennel["compile-string"](f:read("*a"), options)
      end
      _846_0, _847_0 = xpcall(_848_, fennel.traceback)
      if ((_846_0 == true) and (nil ~= _847_0)) then
        local val = _847_0
        print(val)
      elseif (true and (nil ~= _847_0)) then
        local _0 = _846_0
        local msg = _847_0
...
if ((_G.type(_850_0) == "table") and _851_(...)) then
  return repl()
elseif ((_G.type(_850_0) == "table") and (_850_0[1] == "--repl")) then
  return repl()
elseif ((_G.type(_850_0) == "table") and (_850_0[1] == "--compile")) then
  local files = {select(2, (table.unpack or _G.unpack)(_850_0))}
  return compile(files)
elseif ((_G.type(_850_0) == "table") and (_850_0[1] == "-c")) then
  local files = {select(2, (table.unpack or _G.unpack)(_850_0))}
  return compile(files)
elseif ((_G.type(_850_0) == "table") and (_850_0[1] == "--compile-binary") and (nil ~= _850_0[2]) and (nil ~= _850_0[3]) and (nil ~= _850_0[4]) and (nil ~= _850_0[5])) then
  local filename = _850_0[2]
  local out = _850_0[3]
  local static_lua = _850_0[4]
  local lua_include_dir = _850_0[5]
  local args = {select(6, (table.unpack or _G.unpack)(_850_0))}
  local bin = require("fennel.binary")
  options.filename = filename
  options.requireAsInclude = true
  return bin.compile(filename, out, static_lua, lua_include_dir, options, args)
elseif ((_G.type(_850_0) == "table") and (_850_0[1] == "--compile-binary")) then
  local cmd = (arg[0] or "fennel")
  return print((require("fennel.binary").help):format(cmd, cmd, cmd))
elseif ((_G.type(_850_0) == "table") and (_850_0[1] == "--eval") and (nil ~= _850_0[2])) then
  local form = _850_0[2]
  return eval(form)
elseif ((_G.type(_850_0) == "table") and (_850_0[1] == "-e") and (nil ~= _850_0[2])) then
  local form = _850_0[2]
  return eval(form)
else
  ...
  if (((_G.type(_850_0) == "table") and (nil ~= _850_0[1])) and _881_(...)) then
    local a = _850_0[1]
    return print(fennel["runtime-version"]())
  elseif ((_G.type(_850_0) == "table") and (_850_0[1] == "--help")) then
    return print(help)
  elseif ((_G.type(_850_0) == "table") and (_850_0[1] == "-h")) then
    return print(help)
  elseif ((_G.type(_850_0) == "table") and (_850_0[1] == "-")) then
    return dosafely(fennel.eval, (io.stdin):read("*a"))
  elseif ((_G.type(_850_0) == "table") and (nil ~= _850_0[1])) then
    local filename = _850_0[1]
    local args = {select(2, (table.unpack or _G.unpack)(_850_0))}
    arg[-2] = arg[-1]
    arg[-1] = arg[0]
