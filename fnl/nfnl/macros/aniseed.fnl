...
;; Aniseed used to insert a deletion marker into some forms which were then
;; deleted in a post-processing step. That doesn't exist in nfnl so we don't
;; insert the delete marker, it does mean you'll end up with a few more jump
;; noop Lua lines.

;; These are things that were byproducts of the complicated macros, they
;; shouldn't break things but they're a little ugly. It's more trouble than
;; it's worth to remove them now though in nfnl land so they stay.
...
(fn module [mod-name mod-fns mod-base]
  (let [;; So we can check for existing values and know if we're in an interactive eval.
        ...
        result `[;; We can't refer to things like (local (foo bar) (10 foo)).
